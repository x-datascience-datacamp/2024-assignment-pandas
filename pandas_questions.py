...
def merge_referendum_and_areas(referendum, regions_and_departments):
    ...
    referendum_filtered = referendum.loc[~referendum["Department code"].isin(
        excluded_departments)]
...
def compute_referendum_result_by_regions(referendum_and_areas):
    ...
    """
    numeric_columns = ['Registered', 'Abstentions',
                       'Null', 'Choice A', 'Choice B']
    grouped = referendum_and_areas.groupby(
        ["code_reg", "name_reg"], as_index=False)[numeric_columns].sum()
...
def plot_referendum_map(referendum_result_by_regions):
    ...
    * Return a gpd.GeoDataFrame with a column 'ratio' containing the results.
    ...
    geo_merged["ratio"] = geo_merged["Choice A"] / (geo_merged[
        "Choice A"] + geo_merged["Choice B"])
