...
def merge_regions_and_departments(regions, departments):
    ...
    dropped_regions = dropped_regions.rename(columns={'code': 'code_reg',
                                                      'name': 'name_reg'})
    dropped_departments = dropped_departments.rename(
...
def merge_referendum_and_areas(referendum, regions_and_departments):
    ...
    """
    dropped_regions_and_departments = (
        regions_and_departments[regions_and_departments['code_reg'] != 'COM']
        )
    dropped_referendum = (
        referendum[~referendum["Department code"].str[0].str.isalpha()]
        )
    dropped_referendum['code_dep'] = (
        dropped_referendum['Department code'].str.zfill(2)
        )
    referendum_and_areas = (
        pd.merge(dropped_referendum,
                 dropped_regions_and_departments,
                 on='code_dep',
                 how='left')
    )
...
def compute_referendum_result_by_regions(referendum_and_areas):
    ...
    """
    count_by_regions = (
        referendum_and_areas.groupby('name_reg')[['Registered',
                                                  'Abstentions',
                                                  'Null',
                                                  'Choice A',
                                                  'Choice B']].sum()
    ...
    result_by_regions = (
        pd.merge(referendum_and_areas[['code_reg',
                                       'name_reg']],
                 count_by_regions,
                 on='name_reg',
                 how='left')
...
def plot_referendum_map(referendum_result_by_regions):
    ...
    * Return a gpd.GeoDataFrame with a column 'ratio' containing the results.
    ...
    referendum_result_by_regions['total_votes'] = (
