...
def merge_referendum_and_areas(referendum, regions_and_departments):
    ...
    regions_and_departments["code_dep"] = code_dep.apply(replace_useless_zero)
...
def compute_referendum_result_by_regions(referendum_and_areas):
    ...
    """
    selected = ['name_reg', 'Registered', 'Abstentions',
                'Null', 'Choice A', 'Choice B']
    groupby_code_reg = referendum_and_areas.groupby("code_reg")
    counts = groupby_code_reg.agg({'name_reg': 'first',
                                   'Registered': 'sum',
                                   'Abstentions': 'sum',
                                   'Null': 'sum',
                                   'Choice A': 'sum',
